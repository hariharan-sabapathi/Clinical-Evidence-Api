...
column and the retrieval query never know which implementation is behind
them -- exactly the pattern the retrieval library already uses for its LLM
client (src/clinical_retrieval/generation/llm_client.py: ``NullLLMClient``
vs. ``OpenAICompatibleClient``).
...
cosine search, and every test in this repo, exercise the actual code path
a production embedding model would run through. No LLM API key is
configured in this sandbox (see the retrieval library's own README note on
this), so ``OpenAIEmbeddingClient`` is implemented but unexercised here;
set ``LLM_API_KEY`` and wire it in to use it for real.
"""
